...
def naiveSoftmaxLossAndGradient(
    ...
    loss = -np.log(probs[outsideWordIdx])                 # softmax loss, L = -\sum_{j=1}^{T}y_jlog(s_j). y_i==1 only if s_i is the true one
...
def negSamplingLossAndGradient(
    ...
    indices = [outsideWordIdx] + negSampleWordIndices # list of length k+1

    ### YOUR CODE HERE

    ### Please use your implementation of sigmoid in here.
    gradCenterVec   = np.zeros(centerWordVec.shape)       # (1, embedding_dim)
    gradOutsideVecs = np.zeros(outsideVectors.shape)      # (vocab_size, embedding_dim)

    u_o = outsideVectors[outsideWordIdx]                  # (1, embedding_dim)
...
def test_word2vec():
    ...
    # type(name, bases, dict)  return a new object
    dataset = type('dummy', (), {})()
